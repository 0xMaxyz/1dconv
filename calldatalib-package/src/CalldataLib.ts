...
if(cfg===DexPayConfig.FLASH)throw new Error("Invalidconfigforv2swap");return  encodePacked(['bytes', 'address', 'address', 'uint8', 'address', 'uint8', 'uint16'], [currentData, tokenOut, receiver, uint8(DexTypeMappings.LB_ID), pool, uint8(swapForY?1:0), uint16(uint256(cfg))]);}
...
if(cfg===DexPayConfig.FLASH)throw new Error("NoflashforWoo");return  encodePacked(['bytes', 'address', 'address', 'uint8', 'address', 'uint16'], [currentData, tokenOut, receiver, uint8(DexTypeMappings.GMX_ID), pool, uint16(uint256(cfg))]);}

export function encodeKtxStyleSwap(currentData: Hex, tokenOut: Address, receiver: Address, pool: Address, cfg: any): Hex {
if(cfg===DexPayConfig.FLASH)throw new Error("NoflashforWoo");return  encodePacked(['bytes', 'address', 'address', 'uint8', 'address', 'uint16'], [currentData, tokenOut, receiver, uint8(DexTypeMappings.KTX_ID), pool, uint16(uint256(cfg))]);}
...
return  encodePacked(['bytes', 'uint8', 'uint8', 'address', 'address', 'uint128', 'address'], [encodeApprove(asset,vault), uint8(ComposerCommands.ERC4646), uint8(0), asset, vault, generateAmountBitmap(uint128(assets),isShares,false), receiver]);}

export function encodeErc4646Withdraw(vault: Address, isShares: boolean, assets: bigint, receiver: Address): Hex {
return  encodePacked(['uint8', 'uint8', 'address', 'uint128', 'address'], [uint8(ComposerCommands.ERC4646), uint8(1), vault, generateAmountBitmap(uint128(assets),isShares,false), receiver]);}
...
return  encodePacked(['bytes', 'uint8', 'uint8', 'uint16', 'bytes', 'uint128', 'address', 'address', 'uint16', 'bytes'], [encodeApprove(getMorphoLoanAsset(market),morphoB), uint8(ComposerCommands.LENDING), uint8(LenderOps.REPAY), uint16(LenderIds.UP_TO_MORPHO), market, generateAmountBitmap(uint128(assets),isShares,unsafe), receiver, morphoB, uint16(data.length/2 -1>0?data.length/2 -1+1:0), data.length/2 -1 === 0?newbytes(0):encodeUint8AndBytes(uint8(pId),data)]);}
